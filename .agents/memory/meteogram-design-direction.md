...
Temperature and dew-point curves share the same projection, whose visible domain is derived only from temperature. Clip the plotted curves to that temperature viewport, but keep all dew-point numbers fully visible in a compact foreground row directly below the fill. Never distribute the labels vertically or compress the dew curve onto an artificial lower edge. The fill has no colored outline. Do not use horizontal row separators or horizontal chart grid lines; retain only meaningful vertical boundaries such as day changes.