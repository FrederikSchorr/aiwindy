...
The temperature fill must be bounded below by the dew-point curve, with every dew-point value fully visible in front of the fill. Temperature and dew point share one vertical projection; derive its domain only from temperature, but allow the dew curve to extend below that domain without clipping. Do not use a colored outline on the temperature fill. Do not use horizontal row separators or horizontal chart grid lines; retain only meaningful vertical boundaries such as day changes.