...
function weatherIcon(code: number | null, cloudType: CloudType | null) {
  ...
  const common = { viewBox: "0 0 36 32", className: "h-8 w-9", "data-cloud-type-icon": type, "aria-hidden": true };
  ...
  if (type === "clear" && !isRain && !isStorm) return <svg {...common} viewBox="0 0 32 32" className="h-8 w-8"><circle cx="16" cy="16" r="7" fill="#f4b400" /><g stroke="#e7a600" strokeWidth="2" strokeLinecap="round"><path d="M16 3v4M16 25v4M3 16h4M25 16h4M6.8 6.8l2.8 2.8M22.4 22.4l2.8 2.8M25.2 6.8l-2.8 2.8M9.6 22.4l-2.8 2.8" /></g></svg>;
...
function DayHeaders({ points }: { points: MeteogramPoint[] }) {
  ...
  return (
    <div className="grid border-b border-slate-300/55 bg-slate-900/[.025] text-[16px] font-semibold tracking-[.02em] text-slate-700 dark:border-slate-700/50 dark:bg-white/[.025] dark:text-slate-200" style={{ height: ROW.day, gridTemplateColumns: `repeat(${points.length}, ${POINT_WIDTH}px)` }}>
      {groups.map((group, index) => <div key={`${group.label}-${index}`} className="flex items-center justify-start border-r border-slate-300/50 pl-3 dark:border-slate-700/50" style={{ gridColumn: `span ${group.count}` }}><span>{group.label}</span></div>)}
    </div>
  );
}
        ...
        <div className="w-[112px] shrink-0 border-r border-slate-300/75 bg-[#f3f5f6] text-[10px] leading-[11px] text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 md:w-[168px]">
          ...
          <div style={{ height: CLOUD_CHART_HEIGHT }} className="grid grid-cols-[.85fr_1.6fr]">
            ...
            <div className="flex flex-col">
              {data.bands.map((band) => <div key={band.key} className="flex min-h-0 flex-1 flex-col items-center justify-center border-b border-slate-300/45 px-1 text-center dark:border-slate-700/60"><strong className="text-[10px] tracking-[.04em]">{band.label}</strong><span className="text-[9px]">{band.altitude}</span></div>)}
            </div>
          </div>
            ...
            <div className="relative z-10">
              ...
              <div className="relative border-b border-slate-300/50 dark:border-slate-700/50" style={{ height: FORECAST_BLOCK_HEIGHT }}>
                <svg data-testid="meteogram-temperature-area" data-temperature-layer="behind-forecast-rows" viewBox={`0 0 ${chartWidth} ${FORECAST_BLOCK_HEIGHT}`} width={chartWidth} height={FORECAST_BLOCK_HEIGHT} className="pointer-events-none absolute inset-0 z-0 block" role="img" aria-label="Temperaturverlauf">
                  {[12, 24].map((y) => <line key={y} x1="0" x2={chartWidth} y1={y} y2={y} stroke="#b9c2cc" strokeOpacity=".3" strokeDasharray="2 4" />)}
                   ...
                   {temperatureArea && <path data-series="temperature" d={temperatureArea} fill="url(#temperature-gradient)" fillOpacity=".34" mask="url(#temperature-soft-fade)" />}
                   {temperaturePath && <path data-series="temperature" d={temperaturePath} fill="none" stroke="url(#temperature-gradient)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />}
                </svg>
                 <div className="relative z-10 grid" style={{ height: ROW.icons, gridTemplateColumns: grid }}>
                   {data.points.map((point, index) => <div key={`icon-${point.timestamp}-${index}`} data-weather-cloud-type={point.cloudType ?? "unknown"} className="flex items-center justify-center border-r border-slate-300/15" title={`${point.weatherCode === null ? "Wetterzustand nicht verfügbar" : `Wettercode ${point.weatherCode}`} · ${cloudTypeTooltip(point)}`}>{weatherIcon(point.weatherCode, point.cloudType)}</div>)}
                </div>
              ...
              <svg data-testid="meteogram-cloud-field" viewBox={`0 0 ${chartWidth} ${CLOUD_CHART_HEIGHT}`} width={chartWidth} height={CLOUD_CHART_HEIGHT} className="pointer-events-none absolute inset-0 z-[1] block bg-transparent dark:border-slate-700" role="img" aria-label="Mehrschichtige Wolkenbedeckung nach Höhe">
               ...
               <svg data-testid="meteogram-pressure-rain-overlay" viewBox={`0 0 ${chartWidth} ${PRESSURE_RAIN_HEIGHT}`} width={chartWidth} height={PRESSURE_RAIN_HEIGHT} className="pointer-events-none absolute inset-0 z-[2] block bg-transparent dark:border-slate-700" role="img" aria-label="Luftdruck und Regen">
                ...
                {rainGroups.map((group, index) => group.rainTotal >= 0.05 && <div key={`rain-total-${group.label}-${index}`} data-testid="meteogram-daily-rain" data-rain-total={group.rainTotal.toFixed(1)} data-rain-pill-placement="cloud-chart" title={`Tagessumme Niederschlag: ${formatRainAmount(group.rainTotal)}`} aria-label={`Tagessumme Niederschlag ${formatRainAmount(group.rainTotal)}`} className="pointer-events-auto absolute top-2 z-[3] -translate-x-full rounded-full bg-blue-600 px-2 py-0.5 text-[10px] font-bold tracking-normal text-white shadow-sm" style={{ left: (group.startIndex + group.count) * POINT_WIDTH - 10 }}>{formatRainAmount(group.rainTotal)}</div>)}
              </div>
               {hasCloudBase && <div data-testid="meteogram-cloud-base-row" aria-label="Geschätzte Wolkenuntergrenze" className="grid border-b border-slate-300/60 text-[13px] font-medium text-slate-600 dark:text-slate-300" style={{ height: ROW.cloudBase, gridTemplateColumns: grid }}>{data.points.map((point, index) => <div key={`base-${point.timestamp}-${index}`} data-cloud-base-level={point.cloudBase === null ? "unavailable" : point.cloudBase < 300 ? "very-low" : point.cloudBase < 600 ? "low" : point.cloudBase < 1000 ? "caution" : point.cloudBase < 2000 ? "moderate" : point.cloudBase < 5000 ? "high" : "very-high"} className={`flex items-center justify-center border-r border-slate-300/30 ${point.cloudBase === null ? "" : cloudBaseTone(point.cloudBase)}`} title={point.cloudBase === null ? "Keine gültige Schätzung der Wolkenuntergrenze" : `Geschätzte Wolkenuntergrenze: ${Math.round(point.cloudBase)} m, aus Temperatur und Taupunkt abgeleitet; keine Beobachtung`}>{point.cloudBase !== null ? formatCloudBase(point.cloudBase) : "—"}</div>)}</div>}
             </div>
             <div data-testid="meteogram-current-column" aria-hidden="true" className="pointer-events-none absolute z-20 rounded-[5px] border-2 border-rose-400/50 bg-rose-300/[.055]" style={{ left: currentPointIndex * POINT_WIDTH, top: ROW.day, bottom: 0, width: POINT_WIDTH }} />
          </div>
        </div>
      </div>
    </div>
