...
const ROW = { day: 43, hours: 34, icons: 48, temperature: 42, dew: 26, clouds: 210, base: 35 };
    ...
    <div className="flex min-w-0">
      ...
      <div className="meteogram-current__scroll min-w-0 flex-1 overflow-x-auto"><div className="relative" style={{ minWidth: width }}>
        ...
        <div className="relative z-0">
          ...
          <div className="relative" style={{ height: ROW.icons + ROW.temperature }}><svg viewBox={`0 0 ${width} 92`} width={width} height="92" className="pointer-events-none absolute inset-0"><defs><linearGradient id="current-temperature" x2={width} gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={temperatureColor(points[0].temperature)} />{points.map((point, index) => <stop key={point.timestamp} offset={`${index / (points.length - 1) * 100}%`} stopColor={temperatureColor(point.temperature)} />)}</linearGradient></defs><path d={temperatureArea} fill="url(#current-temperature)" fillOpacity=".56" /><path d={temperaturePath} fill="none" stroke="url(#current-temperature)" strokeWidth="2.4" strokeLinecap="round" /><path d={smoothPath(dewBoundaryPoints)} fill="none" stroke="#d18c71" strokeWidth="1.2" strokeLinecap="round" /></svg><div className="relative grid" style={{ height: ROW.icons, ...grid }}>{points.map(point => <div key={point.timestamp} className="flex items-center justify-center" title={`${point.cloudType} · Regen ${point.rain.toFixed(1)} mm`}>{weatherIcon(point)}</div>)}</div><div className="relative grid text-[21px] font-medium text-[#20252a]" style={{ height: ROW.temperature, ...grid }}>{points.map(point => <div key={point.timestamp} className="flex items-center justify-center">{point.temperature}°</div>)}</div></div>
          <div className="grid bg-[#fafbfc] text-[15px] text-[#7b838b]" style={{ height: ROW.dew, ...grid }}>{points.map(point => <div key={point.timestamp} className="flex items-center justify-center">{point.dewPoint}°</div>)}</div>
